...
class OccupancyGridPlanner {
    protected:
        ...
        //End Exploration
        ros::Subscriber end_expl_sub;
        //ros::Publisher end_expl_pub;
        ...
        //End exploration
		void end_expl_cb(std_msgs::Bool end){
			end_exploration = end.data;
			ROS_INFO("Finish exploration, coming back home");		
		}
        ...
        void og_callback(const nav_msgs::OccupancyGridConstPtr & msg) {
            ...
            if (border_points.size()==0){
				//Publish something to say that exploration is done
				end_exploration = true;
				//std_msgs::Bool finished;
				//finished.data
				//end_expl_pub.publish();
				ROS_INFO("Exploration is done");			
			}
            
            ros::Time now=ros::Time::now() ;
            
            if (end_exploration==false){
    ...
    public:
        OccupancyGridPlanner() : nh_("~") {
			...
			//End Exploration
			end_expl_sub=nh_.subscribe<std_msgs::Bool>("end_expl",1,&OccupancyGridPlanner::end_expl_cb,this);
			//end_expl_pub=nh_.advertise<std_msgs::Bool>("end_expl",1,true);
        }
};
