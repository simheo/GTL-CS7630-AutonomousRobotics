...
class ObstacleAvoidance {
    ...
    protected: // ROS Callbacks
        ...
        geometry_msgs::Twist findClosestAcceptableVelocity(const geometry_msgs::Twist & desired) {
			...
			min_v=std::max(current_velocity_.linear.x-max_linear_accel_*time_horizon_,min_v);
			max_v=std::min(current_velocity_.linear.x+max_linear_accel_*time_horizon_,max_v);
			min_w=std::max(current_velocity_.angular.z-max_angular_accel_*time_horizon_,min_w);
			max_w=std::min(current_velocity_.angular.z+max_angular_accel_*time_horizon_,max_w);
            ...
            for (unsigned int j=0;j<n_v;j++) {
                ...
                for (unsigned int i=0;i<n_w;i++) {
                    ...
                    double alpha=atan(v/w);
                    Va(j,i) = occupancy_dalpha(d,alpha);
                    
                    double current_score=255*exp(-(v-desired.linear.x)*(v-desired.linear.x)/(2*k_v_*k_v_)-(w-desired.angular.z)*(w-desired.angular.z)/(2*k_w_*k_w_)) ;
                    scores(j,i) = current_score;
                                        
                    
                    if (current_score>best_score && Va(j,i) != OCCUPIED){
