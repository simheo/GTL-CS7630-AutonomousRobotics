...
class RoverKF(RoverKinematics):
    ...
    def predict(self, motor_state, drive_cfg, encoder_precision):
        ...
        A=mat(
            [
                [1, 0, -sin(theta)*DX[0,0]-cos(theta)*DX[1,0]],
                [0, 1, cos(theta)*DX[0,0]-sin(theta)*DX[1,0]],
                [0, 0, 1]
            ])
        
        # ultimately : 
        displacement=matmul(Rth, DX)
        self.X += displacement 
        self.P = transpose(A)*self.P*A+Q
    ...
    def update_ar(self, Z, L, uncertainty):
        ...
        xdiff=L[0,0]-self.X[0,0]
        ydiff=L[1,0]-self.X[1,0]
        ...
        #kh=matmul(K,H)
        self.X +=K*(Z-h)
        self.P = (mat(identity(3))-K*H)*self.P 
